...
​		Redis单实例的架构，从最开始单机数据持久化，一主N从的读写分离和多机备份，再到Sentinel哨兵处理故障转移的高可用性，单实例的`Redis`缓存足以应对大多数的使用场景。但是每台Redis服务器始终存储相同的数据，==浪费内存，且扩容困难，写操作永远集中在单台服务器上==：

- 写并发的压力仍在：Redis单实例读写分离可以解决==读操作==的负载均衡，但写操作仍全部落在`master`节点上，在海量数据高并发场景，单节点写数据容易出现瓶颈，造成master节点的压力上升。
- 海量数据的存储压力和备份压力：（内存容量的限制）Redis的最大缺点和局限性就在于内存存储数据，海量数据对容量而言会有相当大的限制。（持久化和硬盘的限制）Redis单实例当数据量太大意味着持久化成本高，严重时可能会阻塞服务器，造成服务请求成功率下降，降低服务的稳定性。



#### 1、Cluster 架构

通过`cluster`搭建集群，可以实现主从复制+读写分离+哨兵集群高可用+数据分片的集群架构：

​		①`Redis cluster`内置了高可用机制和读写分离，且解决了单机写操作无法负载均衡。②Redis集群可以看成多个主从架构组合起来的，最少需要三主三从，每个主从架构可以看成一个节点，支持N个`master`节点，每个master节点都可以挂载多个`slave`节点。③Redis集群采用去中心化的思想，当客户端操作的key没有分配到该node上时，Redis会返回转向指令，指向正确的Redis节点。④Redis集群实现了数据的分布式存储，对数据进行分片，将不同的数据存储在不同的master节点上面，解决海量数据的存储问题。⑤Redis集群中的每一个实例需要开放两个端口，比如一个6379，另一个就是就上10000即16379的`cluster bus`，使用二进制协议来让节点间进行高效的数据交换通信。

<img src="img/0b45978abeca4d678fbd3aa39130a4ad.png" alt="34.png" style="zoom: 67%;" />

#### 2、服务器分片算法

**『**客户端端**』**：在客户端实现相关的逻辑，例如用取模或者一致性哈希对key进行分片，查询和修改都先通过路由算法判断key。Jedis客户端提供了Redis Sharding的方案：一致性hash和md5散列两种hash算法，默认一致性hash。并且为了使得请求能在服务器上负载均衡，Sharded Jedis会使用节点的名称虚拟化出160个虚拟节点。当客户端访问redis时，首先根据key计算出其落在哪个节点上，然后找到节点的ip和端口进行连接访问。

**『**中间代理端**』**：把做分片处理的逻辑抽取出来，运行一个独立的代理服务，客户端连接到这个代理服务，代理服务做请求的转发。典型的代理分区方案有 Twitter 开源的 Twemproxy 和国内的豌豆荚开源的 Codis

**『**服务器端**』**：基于服务端实现。

##### 2.1 最古老的hash算法

**原理** ：来了一个key之后，通过`hash(key)%N`对`master node`节点数量取模，根据余数，决定映射到那一个节点。

**存在问题** ：节点新增或者减少，由于取模的`N`发生变化，数据需要重新分布和迁移，导致大量缓存重建的问题。否则一旦某`master`宕机了，所有的请求过来之后，就会对新的节点数量(原节点数量-1)去取模，然后去相应的`node`取数据，导致请求走不到原本路由到的实例上面去，导致大量的key瞬间全部失效无法访问，引起大量的缓存穿透，造成雪崩。

![35.png](https://ucc.alicdn.com/pic/developer-ecology/9a6a6f7bb66d434e84a258d04745c53e.png)

##### 2.2 一致性hash算法

**原理** ：将所有`master node`（ip或名称）落在一个由CRC32哈希算法构建的圆环上0—2^32^-1，当key过来后，同样计算hash值并在哈希圆环上寻找对应点(每个点都有一个hash值)。然后按顺时针方向寻找距离自己最近的节点，之后该key的存储和读取都在该节点进行。

**优势** ：保证了任何一个master宕机，只会影响之前在那个master上面的数据，因为照着顺时针走，在之前的master上面的数据找不到了，就会继续顺着顺时针走到下一个master节点去。这样就只会有一部分数据丢失。

**存在问题** ：假如有3个master，那么就会丢失1/3的数据，所以可能丢失大量数据。 **缓存倾斜问题** ：某个节点占用的hash区间过大，那么就会导致大量的数据进入同一个master，造成该master负载过高。

<img src="https://ucc.alicdn.com/pic/developer-ecology/eadf5aa51fdd463d9024aa41c9218248.png" alt="36.png" style="zoom:67%;" />

**解决方案**：为了解决上面的问题，在一致性哈希的基础上增加了虚拟节点方案来处理。给每个master都做了一部分的虚拟节点，这部分虚拟节点也分布在这个圆环上面，那么在每个区间内，大量的数据就会均匀的分布在不同节点上。

<img src="img/0b46ea4cd8c64665a1b991f8401c23cd.png" alt="37.png" style="zoom: 50%;" />

##### 2.3 哈希槽Slot算法

**原理** ：Redis 集群既没有用哈希取模，也没有用一致性哈希，而是用16384个(0 -16383,2^14^)Hash槽来实现的，每个节点负责一定区间的slot。通过`CRC16`算法计算`key`哈希值并对`16384`取模(`slot = CRC16(key)%16384`) ，随后与对应的槽位的节点进行存取操作。

**优点**：自动更新hash槽的映射数据关系，通过hash槽移动可以方便的添加或者移除节点，且移动hash slot的成本是非常低的。 增加节点时，只需要把其他节点的某些哈希槽挪到新节点；移除节点时，只需要把移除节点上的哈希槽挪到其他节点。由于哈希槽移动并不会停止服务，所以无论添加删除或者改变某个节点的哈希槽的数量都不会造成集群不可用的状态。

<img src="img/a051571209714967abc4a0227d5f3c31.png" alt="38.png" style="zoom: 50%;" />

###### 2.3.1 自定义落点

​		在key里面加入{hash tag}即可。Redis在计算槽编号的时候只会获取{}之间的字符串进行槽编号计算。两个不同的键，但{}里面的字符串是相同的，因此他们可以被计算出相同的槽。

```bash
redis > cluster keyslot somekey
11058
redis > cluster keysolt key2{somekey}
11058
```
...
原文链接：https://blog.csdn.net/FlyLikeButterfly/article/details/120842866
